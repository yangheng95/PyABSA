...
from pyabsa.tasks.text_classification.__bert__.dataset_utils.data_utils_for_inferring import BERTClassificationDataset
from pyabsa.tasks.text_classification.__glove__.dataset_utils.data_utils_for_inferring import GloVeClassificationDataset
...
class TextClassifier:
    def __init__(self, model_arg=None, label_map=None):
        ...
        if not isinstance(model_arg, str):
            ...
            if not hasattr(ClassificationModelList.GloVeClassificationModelList, self.model.__class__.__name__) \
                    and not hasattr(ClassificationModelList.BERTClassificationModelList, self.model.__class__.__name__):
                raise KeyError('The checkpoint you are loading is not from classifier model.')

        if hasattr(ClassificationModelList.BERTClassificationModelList, self.opt.model.__name__):
            self.dataset = BERTClassificationDataset(tokenizer=self.tokenizer, opt=self.opt)

        elif hasattr(ClassificationModelList.GloVeClassificationModelList, self.opt.model.__name__):
