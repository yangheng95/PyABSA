...
from pyabsa.tasks.glove_apc.dataset_utils.data_utils_for_training import (build_tokenizer,
...
class Instructor:
    def __init__(self, opt, logger):
        ...
        if hasattr(ABSADatasets, opt.dataset_path):
            opt.dataset_path = os.path.join('.', opt.dataset_path)
            if not os.path.exists(os.path.join('.', opt.dataset_path)):
                os.mkdir(os.path.join('.', opt.dataset_path))
...
def train4apc_glove(opt, logger):
    ...
    opt.inputs_cols = input_colses[opt.model.__name__.lower()]
    ...
    #     raise RuntimeError('Seems to be a crash among cached datasets, please remove the cached dataset and retry.')
