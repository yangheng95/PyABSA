...
class BERTBaselineABSADataset(Dataset):
    ...
    def __init__(self, dataset_list, tokenizer, opt):
        ...
        for i in tqdm.tqdm(range(0, len(lines), 3), postfix='building word indices...'):
            ...
            context_indices = tokenizer.text_to_sequence('[CLS] ' + text_left + text_right + " [SEP]")
            left_indices = tokenizer.text_to_sequence('[CLS] ' + text_left + " [SEP]")
            left_with_aspect_indices = tokenizer.text_to_sequence('[CLS] ' + text_left + " " + aspect + " [SEP]")
            right_indices = tokenizer.text_to_sequence('[CLS] ' + text_right + " [SEP]", reverse=False)
            right_with_aspect_indices = tokenizer.text_to_sequence('[CLS] ' + aspect + " " + text_right + '[SEP]', reverse=False)
            aspect_indices = tokenizer.text_to_sequence('[CLS] ' + aspect + ' [SEP]')
