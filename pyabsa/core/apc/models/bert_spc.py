...
class BERT_SPC(nn.Module):
    inputs = ["text_bert_indices", "left_text_bert_indices", "right_text_bert_indices"]
    ...
    def __init__(self, bert, opt):
        ...
        self.linear_window_2h = (
            nn.Linear(2 * opt.embed_dim, opt.embed_dim)
            if self.opt.lsa
            else nn.Linear(opt.embed_dim, opt.embed_dim)
        )
        self.linear_window_3h = (
            nn.Linear(3 * opt.embed_dim, opt.embed_dim)
            if self.opt.lsa
            else nn.Linear(opt.embed_dim, opt.embed_dim)
        )
    ...
    def forward(self, inputs):
        ...
        if self.opt.lsa:
            ...
            right_feat = self.bert(inputs["right_text_bert_indices"])[
                "last_hidden_state"
            ]
            if "lr" == self.opt.window or "rl" == self.opt.window:
                if self.opt.eta >= 0:
                    cat_features = torch.cat(
                        (
                            feat,
                            self.opt.eta * left_feat,
                            (1 - self.opt.eta) * right_feat,
                        ),
                        -1,
                    )
                else:
