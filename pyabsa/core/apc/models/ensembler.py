    ...
    def __init__(self, opt, load_dataset=True, **kwargs):
        ...
        for i in range(len(models)):
            config_str = re.sub(
                ...
                str(
                    sorted(
                        [str(self.opt.args[k]) for k in self.opt.args if k != "seed"]
                    )
                ),
            )
            ...
            if hasattr(APCModelList, models[i].__name__):
                try:
                    if kwargs.get("offline", False):
                        ...
                        self.bert = (
                            AutoModel.from_pretrained(self.opt.pretrained_bert)
                            if not self.bert
                            else self.bert
                        )
                ...
                self.tokenizer = (
                    Tokenizer4Pretraining(
                        self.opt.max_seq_len, self.opt.pretrained_bert
                    )
            ...
            if self.opt.cache_dataset and not os.path.exists(cache_path):
                ...
                with open(cache_path, mode="wb") as f:
                    pickle.dump(
                        (self.train_set, self.valid_set, self.test_set, self.opt), f
                    )
