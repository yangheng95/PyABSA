    ...
    def process_data(self, samples, ignore_error=True):
        ...
        for i, text in enumerate(it):
            try:
                ...
                if (
                    self.opt.model_name == "dlcf_dca_bert"
                    or self.opt.model_name == "dlcfs_dca_bert"
                ):
                    configure_dlcf_spacy_model(self.opt)
                    prepared_inputs = prepare_input_for_dlcf_dca(
                        self.opt, self.tokenizer, text_left, text_right, aspect
                    )
                    dlcf_vec = (
                ...
                data = {
                    ...
                    if "lcf_cdw_vec" in self.opt.inputs_cols
                    else 0,
                    "lcf_cdm_vec": lcf_cdm_vec
                    if "lcf_cdm_vec" in self.opt.inputs_cols
                    else 0,
                    "lcfs_vec": lcfs_vec if "lcfs_vec" in self.opt.inputs_cols else 0,
                    "lcfs_cdw_vec": lcfs_cdw_vec
                    if "lcfs_cdw_vec" in self.opt.inputs_cols
                    else 0,
                    "lcfs_cdm_vec": lcfs_cdm_vec
                    if "lcfs_cdm_vec" in self.opt.inputs_cols
                    else 0,
                    "dlcf_vec": dlcf_vec if "dlcf_vec" in self.opt.inputs_cols else 0,
                    "dlcfs_vec": dlcfs_vec
                    if "dlcfs_vec" in self.opt.inputs_cols
                    else 0,
                    "depend_vec": depend_vec
                    if "depend_vec" in self.opt.inputs_cols
                    else 0,
                    "depended_vec": depended_vec
                    if "depended_vec" in self.opt.inputs_cols
                    else 0,
                    "spc_mask_vec": build_spc_mask_vec(self.opt, text_raw_bert_indices)
                    if "spc_mask_vec" in self.opt.inputs_cols
                    else 0,
                    "text_bert_indices": text_bert_indices
                    if "text_bert_indices" in self.opt.inputs_cols
                    else 0,
                    "aspect_bert_indices": aspect_bert_indices
                    if "aspect_bert_indices" in self.opt.inputs_cols
                    else 0,
                    "text_raw_bert_indices": text_raw_bert_indices
                    if "text_raw_bert_indices" in self.opt.inputs_cols
                    else 0,
                    "polarity": polarity,
                }
                ...
                if pad_idx in data["cluster_ids"]:
                    ...
                    cluster_ids.append(
                        self.opt.label_to_index.get(
                            self.opt.index_to_label.get(data["polarity"], "N.A."), -999
                        )
                    )
                else:
