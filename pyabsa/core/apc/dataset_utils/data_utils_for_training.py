    ...
    def __init__(self, fname, tokenizer, opt):
        ...
        for i in tqdm.tqdm(range(0, len(lines), 3), postfix="preparing dataloader..."):
            ...
            data = {
                ...
                if "lcfs_cdw_vec" in opt.inputs_cols
                else 0,
                "lcfs_cdm_vec": lcfs_cdm_vec
                if "lcfs_cdm_vec" in opt.inputs_cols
                else 0,
                ...
                if "depended_vec" in opt.inputs_cols
                else 0,
                "spc_mask_vec": build_spc_mask_vec(opt, text_raw_bert_indices)
                if "spc_mask_vec" in opt.inputs_cols
                else 0,
                "text_bert_indices": text_bert_indices
                if "text_bert_indices" in opt.inputs_cols
                else 0,
                "aspect_bert_indices": aspect_bert_indices
                if "aspect_bert_indices" in opt.inputs_cols
                else 0,
                "text_raw_bert_indices": text_raw_bert_indices
                if "text_raw_bert_indices" in opt.inputs_cols
                else 0,
                "polarity": polarity,
            }
            ex_id += 1
