...
from pyabsa.core.apc.classic.__glove__.dataset_utils.data_utils_for_training import (
    build_tokenizer,
)
...
from pyabsa.core.apc.classic.__bert__.dataset_utils.data_utils_for_inference import (
    BERTBaselineABSADataset,
)
from pyabsa.core.apc.classic.__glove__.dataset_utils.data_utils_for_inference import (
    GloVeABSADataset,
)
...
class SentimentClassifier:
    def __init__(self, model_arg=None, cal_perplexity=False, **kwargs):
                ...
                if state_dict_path or model_path:
                    if state_dict_path:
                        self.model = APCEnsembler(
                            self.opt, load_dataset=False, **kwargs
                        )
                    ...
                    with open(tokenizer_path, mode="rb") as f:
                        if hasattr(APCModelList, self.opt.model.__name__):
                            ...
                            if tokenizer_path:
                                ...
                                raise ValueError(
                                    "No .tokenizer file found in checkpoint path!"
                                )
                        else:
                            tokenizer = build_tokenizer(
                                ...
                                dat_fname="{0}_tokenizer.dat".format(
                                    os.path.basename(self.opt.dataset_name)
                                ),
                                opt=self.opt,
                            )
        ...
        if isinstance(self.opt.model, list):
            if hasattr(APCModelList, self.opt.model[0].__name__):
                ...
                self.dataset = BERTBaselineABSADataset(
    ...
    def infer(
        ...
        self.infer_dataloader = DataLoader(
            dataset=self.dataset, batch_size=self.opt.eval_batch_size, shuffle=False
        )
    ...
    def _infer(self, save_path=None, print_result=True):
        ...
        with torch.no_grad():
            ...
            for _, sample in enumerate(it):
                inputs = {
                    col: sample[col].to(self.opt.device)
                ...
                for i, i_probs in enumerate(t_probs):
                    if (
                        ...
                        and int(i_probs.argmax(axis=-1)) in self.opt.index_to_label
                    ):
                        sent = self.opt.index_to_label[int(i_probs.argmax(axis=-1))]
                        real_sent = (
                            sample["polarity"][i]
                            if isinstance(sample["polarity"][i], str)
                            else self.opt.index_to_label.get(
                                int(sample["polarity"][i]), "N.A."
                            )
                        )
            ...
            if print_result:
                for show_id, result in enumerate(results):
                    ...
                    for i in range(len(result["aspect"])):
                        if (
                            result["ref_sentiment"][i] != -999
                            and result["ref_sentiment"][i] != "-999"
                        ):
