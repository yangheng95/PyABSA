...
import numpy as np
import tqdm
from pyabsa.core.apc.classic.__bert__.dataset_utils.classic_bert_apc_utils import (
    build_sentiment_window,
)

from pyabsa.core.apc.classic.__bert__.dataset_utils.dependency_graph import (
    ...
    def process_data(self, samples, ignore_error=True):
        ...
        for ex_id, text in enumerate(it):
            try:
                ...
                if "!sent!" in text:
                    text, polarity = (
                        text.split("!sent!")[0].strip(),
                        text.split("!sent!")[1].strip(),
                    )
                ...
                prepared_inputs = prepare_input_for_apc(
                    self.opt, self.tokenizer.tokenizer, text_left, text_right, aspect
                )
                ...
                right_indices = self.tokenizer.text_to_sequence(
                    text_right, reverse=False
                )
                right_with_aspect_indices = self.tokenizer.text_to_sequence(
                    aspect + " " + text_right, reverse=False
                )
                aspect_indices = self.tokenizer.text_to_sequence(aspect)
                aspect_len = np.sum(aspect_indices != 0)
                left_len = min(
                    self.opt.max_seq_len - aspect_len, np.sum(left_indices != 0)
                )
                left_indices = np.concatenate(
                    (
                        left_indices[:left_len],
                        np.asarray([0] * (self.opt.max_seq_len - left_len)),
                    )
                )
                ...
                data = {
                    ...
                    if "text_bert_indices" in self.opt.inputs_cols
                    else 0,
                    "context_indices": context_indices
                    if "context_indices" in self.opt.inputs_cols
                    else 0,
                    "left_indices": left_indices
                    if "left_indices" in self.opt.inputs_cols
                    else 0,
                    "left_with_aspect_indices": left_with_aspect_indices
                    if "left_with_aspect_indices" in self.opt.inputs_cols
                    else 0,
                    "right_indices": right_indices
                    if "right_indices" in self.opt.inputs_cols
                    else 0,
                    "right_with_aspect_indices": right_with_aspect_indices
                    if "right_with_aspect_indices" in self.opt.inputs_cols
                    else 0,
                    "aspect_indices": aspect_indices
                    if "aspect_indices" in self.opt.inputs_cols
                    else 0,
                    "aspect_boundary": aspect_boundary
                    if "aspect_boundary" in self.opt.inputs_cols
                    else 0,
                    "aspect_position": aspect_position,
                    "dependency_graph": dependency_graph
                    if "dependency_graph" in self.opt.inputs_cols
                    else 0,
                    "text_raw": text,
                    "aspect": aspect,
                    "polarity": polarity,
                ...
                all_data = build_sentiment_window(
                    all_data,
                    self.tokenizer,
                    self.opt.similarity_threshold,
                    input_demands=self.opt.inputs_cols,
                )
                for data in all_data:
                    ...
                    for pad_idx in range(self.opt.max_seq_len):
                        if pad_idx in data["cluster_ids"]:
                            # print(data['polarity'])
                            cluster_ids.append(
                                self.opt.label_to_index.get(
                                    self.opt.index_to_label.get(
                                        data["polarity"], "N.A."
                                    ),
                                    -999,
                                )
