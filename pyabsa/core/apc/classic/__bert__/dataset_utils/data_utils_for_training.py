...
class BERTBaselineABSADataset(Dataset):
    def __init__(self, dataset_list, tokenizer, opt):
        ...
        dep_cache_path = os.path.join(
            os.getcwd(), "run/{}/dependency_cache/".format(opt.dataset_name)
        )
        if not os.path.exists(dep_cache_path):
            os.makedirs(dep_cache_path)
        graph_path = prepare_dependency_graph(
            dataset_list, dep_cache_path, opt.max_seq_len
        )
        ...
        for i in tqdm.tqdm(range(0, len(lines), 3), postfix="preparing dataloader..."):
            ...
            prepared_inputs = prepare_input_for_apc(
                opt, tokenizer.tokenizer, text_left, text_right, aspect
            )
            ...
            left_indices = np.concatenate(
                (
                    ...
                    np.asarray([0] * (opt.max_seq_len - left_len)),
                )
            )
            ...
            data = {
                ...
                if "text_bert_indices" in opt.inputs_cols
                else 0,
                "context_indices": context_indices
                if "context_indices" in opt.inputs_cols
                else 0,
                "left_indices": left_indices
                if "left_indices" in opt.inputs_cols
                else 0,
                "left_with_aspect_indices": left_with_aspect_indices
                if "left_with_aspect_indices" in opt.inputs_cols
                else 0,
                "right_indices": right_indices
                if "right_indices" in opt.inputs_cols
                else 0,
                "right_with_aspect_indices": right_with_aspect_indices
                if "right_with_aspect_indices" in opt.inputs_cols
                else 0,
                "aspect_indices": aspect_indices
                if "aspect_indices" in opt.inputs_cols
                else 0,
                "aspect_boundary": aspect_boundary
                if "aspect_boundary" in opt.inputs_cols
                else 0,
                "aspect_position": aspect_position,
                "dependency_graph": dependency_graph
                if "dependency_graph" in opt.inputs_cols
                else 0,
                "polarity": polarity,
            }
            ex_id += 1
        ...
        all_data = build_sentiment_window(
            all_data, tokenizer, opt.similarity_threshold, input_demands=opt.inputs_cols
        )
