...
class TextClassifier:
    def __init__(self, model_arg=None, cal_perplexity=False, **kwargs):
        ...
        if not isinstance(model_arg, str):
                        ...
                        if model_path:
                            ...
                            embedding_matrix = build_embedding_matrix(
                                ...
                                dat_fname="{0}_{1}_embedding_matrix.dat".format(
                                    str(self.opt.embed_dim),
                                    os.path.basename(self.opt.dataset_name),
                                ),
                                opt=self.opt,
                            )
                            self.model = self.opt.model(embedding_matrix, self.opt).to(
                                self.opt.device
                            )
                            self.model.load_state_dict(
                                torch.load(state_dict_path, map_location="cpu"),
            ...
            if not hasattr(GloVeTCModelList, self.opt.model.__name__) and not hasattr(
                BERTTCModelList, self.opt.model.__name__
            ):
                raise KeyError(
        ...
        if hasattr(BERTTCModelList, self.opt.model.__name__):
            self.dataset = BERTClassificationDataset(
                ...
                for i, i_probs in enumerate(t_probs):
                    if (
                        ...
                        if sample["label"][i] != -999:
                            real_sent = (
                                sample["label"][i]
                                if isinstance(sample["label"][i], str)
