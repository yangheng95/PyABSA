...
def preprocess_rna():
    ...
    train_data = load_file(
        "integrated_datasets/rnac_datasets/degrad/degrad.train.dat.rnac"
    )
    test_data = load_file(
        "integrated_datasets/rnac_datasets/degrad/degrad.test.dat.rnac"
    )
    valid_data = load_file(
        "integrated_datasets/rnac_datasets/degrad/degrad.valid.dat.rnac"
    )

    positive_data = load_file(
        "integrated_datasets/rnac_datasets/degrad-v2/Degrad_XRN4_DL_1.tsv"
    )
    negative_data = load_file(
        "integrated_datasets/rnac_datasets/degrad-v2/Degrad_XRN4_DL_0.tsv"
    )
    positive_rna_label = dict()
    negative_rna_label = dict()
    for line in positive_data:
        if line.split("\t")[0].strip() not in positive_rna_label:
            positive_rna_label[line.split("\t")[-1].strip()] = [
                line.split("\t")[2].strip()
            ]
        else:
            positive_rna_label[line.split("\t")[-1].strip()].append(
                line.split("\t")[2].strip()
            )

    for line in negative_data:
        if line.split("\t")[0].strip() not in negative_rna_label:
            negative_rna_label[line.split("\t")[-1].strip()] = [
                line.split("\t")[2].strip()
            ]
        else:
            negative_rna_label[line.split("\t")[-1].strip()].append(
                line.split("\t")[2].strip()
            )
    ...
    with open(
        "integrated_datasets/rnac_datasets/degrad-v2/degrad-v2.train.dat.rnac", "w"
    ) as f:
        for line in train_data:
            ...
            if line.split("$LABEL$")[0].strip() in positive_rna_label:
                line = (
                    line.strip()
                    + ","
                    + positive_rna_label[line.split("$LABEL$")[0].strip()][0]
                    + "\n"
                )
            else:
                line = (
                    line.strip()
                    + ","
                    + negative_rna_label[line.split("$LABEL$")[0].strip()][0]
                    + "\n"
                )
            f.write(line)

    with open(
        "integrated_datasets/rnac_datasets/degrad-v2/degrad-v2.test.dat.rnac", "w"
    ) as f:
    ...
    with open(
        "integrated_datasets/rnac_datasets/degrad-v2/degrad-v2.valid.dat.rnac", "w"
    ) as f:
    ...
    with open(
        "integrated_datasets/rnac_datasets/degrad-v2/degrad-v2.test.dat.rnac.inference",
        "w",
    ) as f:
        for data in positive_test_names + negative_test_names:
            if data in positive_test_names:
