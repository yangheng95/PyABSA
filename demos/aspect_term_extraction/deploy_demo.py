...
dataset_dict = {
    dataset.name: get_example(dataset.name) for dataset in ABSADatasetList()
}
...
with demo:
    ...
    gr.Markdown(
        """### Repo: [PyABSA](https://github.com/yangheng95/PyABSA)
    ...
    gr.Markdown(
        "Your input text should be no more than 80 words, that's the longest text we used in training. However, you can try longer text in self-training "
    )
    gr.Markdown(
        "**You don't need to split each Chinese (Korean, etc.) token as the provided, just input the natural language text.**"
    )
    output_dfs = []
    with gr.Row():
        with gr.Column():
            ...
            dataset_ids = gr.Radio(
                choices=[dataset.name for dataset in ABSADatasetList()[:-1]],
                value="Laptop14",
                label="Datasets",
            )
    ...
    gr.Markdown(
        "![visitor badge](https://visitor-badge.glitch.me/badge?page_id=https://huggingface.co/spaces/yangheng/Multilingual-Aspect-Based-Sentiment-Analysis)"
    )
