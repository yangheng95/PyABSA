...
sentiment_classifier = APCCheckpointManager.get_sentiment_classifier(
    checkpoint="multilingual", auto_device=True  # False means load model on CPU
)


def inference(text):
    result = sentiment_classifier.infer(
        text=text, print_result=True, ignore_error=False, clear_input_samples=True
    )
    ...
    result = pd.DataFrame(
        {
            ...
            "ref_sentiment": [
                "" if ref == "-999" else ref for ref in result["ref_sentiment"]
            ],
...
if __name__ == "__main__":
    iface = gr.Interface(
        ...
        examples=[
            [
                "Strong build though which really adds to its [ASP]durability[ASP] ."
            ],  # !sent! Positive
            [
